...
void Player::moveOrAttack(int dir)
{
        ...
        switch (dir)
    {
        case UP:
            ...
            {m_arena-> attackRobotAt(row() - 1, col(), dir);
                return;
            }
        break;
        case DOWN:
        ...
        case LEFT:
            ...
            {m_arena-> attackRobotAt(row(), col() - 1, dir);
                return;
            }
        break;
        case RIGHT:
...
{
    return m_nRobots - m_nRobotsKilled;
}
...
void Arena::display() const
{
    ...
    for (int i = 0; i < m_nRobots; i++)
        {   if (m_robots[i] == nullptr)
            continue;
            char& ChInTheGird = grid[m_robots[i]->row() - 1][m_robots[i]-> col() - 1];
            switch (ChInTheGird)
            {
                case '.':
                    ChInTheGird = 'R';
...
{
    // Create a game
    // Use this instead to create a mini-game:   Game g(3, 4, 2);  7 8 25
    Game g(7, 8, 25);
